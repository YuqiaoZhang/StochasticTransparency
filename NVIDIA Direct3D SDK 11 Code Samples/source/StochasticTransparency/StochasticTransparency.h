...
public:
	...
	virtual void Render(ID3D11DeviceContext* pd3dImmediateContext, ID3D11RenderTargetView *pBackBuffer)
	{
		...
		//For Stochastic Transparency, each geometry pass uses 8 MSAA samples per pixel and multiple passes can be performed to simulate more MSAA samples per pixel.
		//由于效率原因，一般只使用一个Pass
