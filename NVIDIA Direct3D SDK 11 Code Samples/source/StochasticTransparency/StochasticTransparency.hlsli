...
Pixel_PSOut1 StochasticDepthPS ( Geometry_VSOut IN, uint PrimitiveID : SV_PrimitiveID )
{
	...
	Pixel_PSOut1 rtval;
	rtval.depth = IN.HPosition.z; //interpolation centroid not sample //三角形交界处仍旧有接缝，可能与Tie-Break法则有关，尝试保守光栅化？？？
...
Pixel_PSOut AccumulateAndTotalAlphaPS( Geometry_VSOut IN )
{
	...
	//if ((IN.HPosition.x - pos2d.x) > 0.5f) { pos2d.x += 1; }
	//if ((IN.HPosition.y - pos2d.y) > 0.5f) { pos2d.y += 1; }
	...
	for (uint sampleId = 0; sampleId < NUM_MSAA_SAMPLES; ++sampleId)
	{
		...
		if (z <= (zi /*+ 0x400/*Depth Bias*/)) //Depth Bias 忽略最后10位
		{
			++count;
		}
	}
